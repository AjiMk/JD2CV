...
import { FiFileText, FiUser, FiBookOpen, FiBriefcase, FiAward, FiMenu, FiX, FiEye, FiMaximize2, FiMinimize2, FiMoon, FiSun, FiMove } from 'react-icons/fi'
...
export default function ResumeBuilderPage() {
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      const safeName = (personalInfo.fullName || user?.name || 'Resume').replace(/\s+/g, '_')
  ...
  const previewScale = useMemo(() => Math.max(0.64, Math.min(1.0, splitRatio / 58)), [splitRatio])
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <nav className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-b fixed w-full z-10 shadow-sm rounded-b-2xl overflow-hidden`}>
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-14 items-center">
            <div className="flex items-center gap-3">
              <button onClick={() => setSidebarOpen(!sidebarOpen)} className={`lg:hidden p-2 rounded-md ${darkMode ? 'text-gray-400 hover:text-gray-300 hover:bg-gray-700' : 'text-gray-400 hover:text-gray-500 hover:bg-gray-100'}`}>
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 w-full pt-20">
        ...
        <aside className={`fixed lg:static inset-y-0 left-0 z-20 w-48 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-r pt-14 lg:pt-0 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out overflow-y-auto rounded-2xl`}>
          ...
          <div className={`px-2 py-2 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'} mt-2`}>
            <h3 className={`text-xs font-semibold ${darkMode ? 'text-gray-300' : 'text-gray-700'} mb-2 uppercase tracking-wide`}>Builder Progress</h3>
            <div className="space-y-1.5 text-xs">
              <Row label="Personal Info" done={Boolean(personalInfo.fullName)} darkMode={darkMode} />
              <Row label="Education" done={education.length > 0} darkMode={darkMode} />
              <Row label="Experience" done={workExperience.length > 0} darkMode={darkMode} />
              <Row label="Skills" done={skills.technical.length > 0} darkMode={darkMode} />
              <Row label="Projects" done={projects.length > 0} darkMode={darkMode} />
            </div>
          </div>
        </aside>

        <div ref={containerRef} className="flex-1 flex flex-col lg:flex-row overflow-hidden text-xs">
          <main
            className={`flex-1 p-2 lg:p-3 overflow-y-auto text-xs ${previewFullscreen ? 'hidden lg:block' : ''}`}
            style={!previewFullscreen ? { flexBasis: `${splitRatio}%` } : undefined}
          >
          ...
          {showPreview && (
            <>
              ...
              <aside
                ...
                style={!previewFullscreen ? { flexBasis: `${100 - splitRatio}%` } : undefined}
              >
              <div className={`p-2 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-300'} border-b flex items-center justify-between rounded-t-2xl`}>
