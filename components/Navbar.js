...
export default function Navbar({ children, transparent = false }) {
  return (
    <nav className={`${transparent ? 'bg-transparent' : 'bg-white shadow-sm'} sticky top-0 z-40 transition-all duration-200`}>
...
export function NavbarBrand({ href = '/', title = 'JD2CV' }) {
  return (
    <a href={href} className="flex items-center group">
      ...
      <span className="ml-2 text-2xl font-bold text-gray-900 group-hover:text-primary-600 transition-colors">
        {title}
      </span>
    </a>
  )
...
export function NavbarLink({ href, children, variant = 'default' }) {
  const variants = {
    ...
    outline: 'px-4 py-2 border-2 border-primary-600 text-primary-600 rounded-lg hover:bg-primary-50',
  }
